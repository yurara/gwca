...
HWND MainWnd;
//Controls
HWND RunStop,LoadDll,Script,Engine,Browse,Compass;
...
void DrawCompass(){
    ...
    //compass
    graphics.DrawEllipse(&blue, 200, 335, 200, 200);
	//self
	...
	for(unsigned int i=1;i<maxAgent;i++){
		...
		if(distance>0){
			angle=GetAngleFromAgentToAgent(i,myId);
...
LRESULT CALLBACK wnd_proc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_COMMAND:
		...
		case START:	// button "Start script"
			MessageBox(hwnd,"Not Implemented yet","Information", MB_OK|MB_ICONINFORMATION);
			break;
...
void update_wnd(){
	...
	while(true){
		...
		//Compass
		if(SendMessage(Compass,BM_GETCHECK,0,0) == BST_CHECKED){
			frame=0;
			DrawCompass();
...
void create_wnd_content(HWND parent)
{
	...
	//Compass:
	Compass = CreateWindowEx(0x00000000, "Button", "Show", WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_AUTOCHECKBOX, 168, 320, 96, 16, parent, (HMENU) CHECK, instance, NULL);
	SendMessage(Compass, WM_SETFONT, (WPARAM) h_font, TRUE);
	//Target Info:
	SendMessage(wnd, WM_SETFONT, (WPARAM) h_font, TRUE);
